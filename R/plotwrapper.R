...
plotwrapper <- function(type=c("smoothplot", "highlowplot", "areaplot", "plotDensity","getPlotCapitalGain", "densityGain","plotDensityPortefeuilleByShare", "getPortefeuilleValue", "varianceGain"), ticker="GOOG", portefe="AC.PA ACA.PA", nomb="75 25", from="2013-01-01", to=Sys.time(), current=FALSE, moyenne=FALSE, variance=FALSE, max=FALSE, min=FALSE){
	...
	if(isTRUE(current)){
		...
		myplot <- myplot + geom_label(x=-Inf, y = maxi, size=5, label = paste("Valeur en temps réelle en devise locale:", maxi,"; Gain Maximal:", gain ,"%" ), hjust = -1, vjust = -0.5, color="green");
	}
		...
		myplot <- myplot + geom_label(x=-Inf, y = mini, size=5, label = paste("Valeur en temps réelle en devise locale:", mini, "; Perte maximale:",perte,"%"), hjust = -1, vjust = -0.5, color="red");
	}
